...
def one(args):
    ...
    try:
        ...
        tab=diffusion_table(nmod, Ne, tig, GRID); ltig=np.log(tig)
        LL=[]; POS=[]
        ...
        return dict(row, MAP_diff=MAP, boot_sd_diff=float(bm.std(ddof=1)),
                    err_diff=MAP-age, n_used_diff=LL.shape[0])
...
if __name__=="__main__":
    ...
    print(f"[diff] {len(rows)} sims, {a.procs} procs", flush=True)
    t0=time.time()
    with mp.Pool(a.procs, maxtasksperchild=2) as pool:
        res=pool.map(one, [(r,a.simdir) for r in rows], chunksize=1)
    cols=list(rows[0].keys())+["MAP_diff","boot_sd_diff","err_diff","n_used_diff","fail"]
    with open(a.out,"w") as f:
        f.write("\t".join(cols)+"\n")
        for r in res: f.write("\t".join(str(r.get(c,"")) for c in cols)+"\n")
    print(f"[diff] done in {(time.time()-t0)/60:.1f} min -> {a.out}", flush=True)
